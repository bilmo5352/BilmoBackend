...
def search_single_website(website: str, query: str, headless: bool = True) -> Dict:
    ...
    try:
        ...
        end_time = time.time()
        print(f"✅ {website} search completed in {end_time - start_time:.2f}s")
...
def read_scraper_output(website: str, query: str) -> Dict:
    ...
    for filename in possible_files:
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                data = json.load(f)
                print(f"✅ Read {website} data from {filename}")
                return data
    ...
    print(f"⚠️ No data file found for {website}")
...
def unified_search(query: str, headless: bool = True) -> Dict:
    ...
    for product in all_products:
        ...
        if "price" in product and product["price"]:
            try:
                price_str = str(product["price"]).replace("₹", "").replace("Rs.", "").replace(",", "").strip()
                # Handle price ranges like "₹25,999 - ₹30,999"
                if "-" in price_str:
...
def display_summary(result):
    ...
    if result['summary']['price_range']['min']:
        print(f"Price Range: ₹{result['summary']['price_range']['min']:.0f} - ₹{result['summary']['price_range']['max']:.0f} ({result['summary']['price_range']['count']} products)")
...
def main():
    ...
    print(f"💾 Results processed for MongoDB storage")
